...
`server.py`(motion_llm 파생: stage 상태기계·dance 절대시각 싱크·자막·자동종료 상속)
+ `rc_fleet.py`(USB 열거·Barrier 동시 발사·대별 집계) + `rc_serial.py`/`rc_backend.py`
(motion_llm 과 동일). 상세 배경: ../motion_llm/docs/RC_WIRED_COMMAND_PLAN.md
