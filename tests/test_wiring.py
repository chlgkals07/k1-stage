"""app.State 는 core.Stage 에 이 앱의 것을 꽂는 배선이다 — 도메인이 서버에 다시 생기면 안 된다.
...
class WiringTest(unittest.TestCase):
    def test_state_is_a_stage(self):
        self.assertIs(app.State.__mro__[1], Stage)

    def test_state_defines_nothing_but_init_and_the_app_only_method(self):
        own = {n for n, v in vars(app.State).items()
    ...
    def test_no_stage_method_is_shadowed(self):
        ...
        shadowed = {n for n, v in vars(app.State).items()
    ...
    def test_init_takes_what_callers_pass(self):
        ...
        params = list(inspect.signature(app.State.__init__).parameters)
