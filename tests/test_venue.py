...
class RobotDeployTest(unittest.TestCase):
    ...
    def test_robot_mode_skips_venue_entirely(self):
        ...
        self.assertNotEqual(call, -1, "가드 뒤에서 check_venue 를 부르지 않는다 — 가드 밖에서 불릴 수 있다")
        self.assertLess(call - guard, 400, "가드와 호출이 떨어져 있다 — 가드 밖에서 불릴 수 있다")
        self.assertEqual(source.count("check_venue(args.venue"), 1)
