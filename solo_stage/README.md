...
현재 카탈로그는 **이번 무대 동작 14개**와 제어 상태 2개만 둔다. 최신 실물 mode 목록을
확인한 뒤에만 이 목록을 바꾼다.
...
| 목록 | 뜻 | 현재 |
|---|---|---|
| `api_allowlist` | gateway가 실행을 허용하는 전부. 운영자 수동 버튼·dance가 여기서 나온다 | **14개** |
| `pad_allowlist` | 그중 **관객 아이패드에 여는 것** | **12개** |

새 동작은 **수동으로 먼저 실물 검증한 뒤** 이 두 목록에 함께 넣는다.
