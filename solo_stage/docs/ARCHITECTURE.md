...
- Python 단위 테스트 통과 (현재 137개 — [STATUS.md](STATUS.md) 참고)
...
| `tools/api_arm_probe.py` | 모션 없이 heartbeat/API authority를 점검하는 개발용 도구 |
| `tests/test_gateway.py` | gateway lifecycle 단위 테스트 |
| `tests/test_ui_http.py` | token→cookie→운영자 화면과 보호 endpoint 회귀 테스트 |
| `tests/test_relay_backend.py` | PC relay 전달 및 allowlist 테스트 |
| `tests/test_dance.py` | 무대 싱크·세대 번호·자동 종료 테스트 |
...
`RobotBackend`가 ROS heartbeat를 생성하므로 운영 중에는 별도의 `tools/api_arm_probe.py` heartbeat를 동시에 실행하지 않는다.
