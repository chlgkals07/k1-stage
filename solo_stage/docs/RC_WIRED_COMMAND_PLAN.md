...
shape17→shape3 dance 병합 선례를 따름). solo_stage에는 P3(백엔드 통합)에서만
`rc_serial.py`/`rc_backend.py`/`rc_slots.yaml`을 가산 병합하고 `server.py`에 토글
분기를 넣는다. 그 전까지 solo_stage 코드는 무접촉 (이 문서 제외).
...
- **사본** `~/Projects/shape3/k1-stage/rc_link/solo_stage_rc/` (정본 무접촉). `--rc` 플래그로
...
### 3.2 PC 쪽 — solo_stage에 가산적 추가 (정본 `shape3/solo_stage`)
