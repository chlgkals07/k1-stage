...
> **음성 대화 시절(2026-08-05 ~ 08-17)의 설계 문서다.** 그 기능은 2026-08-18에 제거됐다 —
> 경위는 [STATUS.md §8](../STATUS.md#8-음성-대화-2026-08-18-제거).
> 다만 여기 적힌 **행동 라이브러리 설계 원칙과 권한 분리 구조**는 지금의 패드·무대 운영에도
> 그대로 쓰인다. `pad_allowlist`가 `llm_allowlist`와 같은 발상의 후속이다.
...
첫 연결에는 Zenoh를 넣지 않는다. 로봇 PC에서 `solo_stage` 서버를 ROS2 gateway로
