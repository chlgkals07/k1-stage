# fleet 모드 — K1 RC 군무 서버

**연결된 RadioMaster Pocket 전부에 명령을 동시에 쏜다** — 꽂은 라디오 수 = 같이
움직이는 로봇 수. PC→USB→Pocket→ELRS→로봇, 로봇 소프트웨어 수정 없음.
단일 로봇·Wi-Fi(relay) 운영은 solo 모드([docs/solo](../solo/APP.md))를 쓴다. fleet 모드는 RC 전용이다.

## 실행

```bash
python3 app.py --mode fleet          # http://<pc>:19000 → /operator 로 리다이렉트
python3 app.py --mode fleet --mock   # 라디오 없이 UI/무대 흐름 확인
```
...
`app.py --mode fleet`(solo 와 같은 앱: stage 상태기계·dance 절대시각 싱크·자막·자동종료는 `core/stage.py`)
+ `runtime/rc_fleet.py`(USB 열거·Barrier 동시 발사·대별 집계) + `runtime/rc_serial.py`/`rc_backend.py`
(solo 와 같은 파일; TLM 대기만 플릿은 0.6초). 데이터는 `config/fleet/`. 상세 배경: [RC_WIRED_COMMAND_PLAN](../solo/RC_WIRED_COMMAND_PLAN.md)
