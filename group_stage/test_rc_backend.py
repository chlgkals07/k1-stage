...
class TestServerIntegration(unittest.TestCase):
    ...
    def test_state_rescan_rc(self):
        import server
        import rc_fleet
        orig = rc_fleet.discover_ports
        rc_fleet.discover_ports = lambda: []
        try:
            fleet = rc_fleet.RcFleetBackend(MOTIONS)
            st = server.State(fleet)
            out = st.rescan_rc()
            self.assertFalse(out["ok"])            # 0대 → 실패 + 안내
            self.assertIn("Pocket", out["msg"])
            # 재탐색 대상이 아닌 백엔드(mock)는 무해하게 통과
            st2 = server.State(server.MockBackend())
            out2 = st2.rescan_rc()
