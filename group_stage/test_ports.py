...
from test_rc_fleet import make_fleet
...
def backends():
    """{이름: 인스턴스}"""
    return {
        "mock": server.MockBackend(),
        "rc": RcBackend(HERE / "motions.yaml", serial=FakeSerial()),
...
class UiListsDoNotDependOnTheBackendTest(unittest.TestCase):
    ...
    def test_lists_are_identical_whatever_the_backend_is(self):
        ...
        base = self.state(server.MockBackend())
    ...
    def test_a_backend_api_allowlist_attribute_is_ignored(self):
        ...
        class Lying(server.MockBackend):
            api_allowlist = {"MimicSquat"}

        self.assertEqual(self.state(Lying()).pad_order, self.state(server.MockBackend()).pad_order)
        self.assertEqual(self.state(Lying()).allowed, self.state(server.MockBackend()).allowed)

    def test_the_policy_still_caps_the_lists(self):
        """자르는 일은 그대로다. 출처가 백엔드가 아니라 정책이 됐을 뿐이다."""
        st = server.State(server.MockBackend(), pad_allowlist=["MimicBowNavel", "MimicSquat"],
                          api_allowlist=["MimicBowNavel"])
        self.assertEqual((st.pad_order, st.pad_allowed), (["MimicBowNavel"], {"MimicBowNavel"}))
        st = server.State(server.MockBackend(), api_allowlist=["MimicBowNavel"])
...
class Capable(server.MockBackend):
...
class StartDanceUsesCapabilitiesTest(unittest.TestCase):
    ...
    def test_a_backend_without_prepare_does_not(self):
        self.assertIsNone(self.start(server.MockBackend())._dance_prep_timer)

    def test_a_longer_motion_extends_the_autostop(self):
        base = self.start(server.MockBackend())._dance_autostop_timer.interval
...
class RescanUsesDiscoveryTest(unittest.TestCase):
    """group 전용: /rc/rescan 은 재탐색 능력이 있는 백엔드에서만 재탐색한다."""

    def test_a_backend_without_discovery_is_not_rescanned(self):
        out = server.State(server.MockBackend()).rescan_rc()
    ...
    def test_a_fleet_is_rescanned(self):
        ...
        out = server.State(fleet).rescan_rc()
