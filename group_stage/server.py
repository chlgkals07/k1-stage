#!/usr/bin/env python3
"""K1 RC 군무 서버 (group_stage) — 연결된 RC 전부에 동시 발사 · display · dance 싱크.

  운영자(/operator) ──버튼──> 이 서버 ──> RcFleetBackend ──> USB Pocket ×N ──ELRS──> 로봇 N대
  TV(/display)      ──폴링──> stage 상태 (idle/executing/dance)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        path = parsed.path
        if path == "/":
            # RC 군무 앱의 기본 화면은 운영자다.
            query = urllib.parse.urlsplit(self.path).query
            self.send_response(302)
            self.send_header("Location", "/operator" + (f"?{query}" if query else ""))
            self.send_header("Content-Length", "0")
            self.end_headers()
            return
        if path in {"/operator", "/display", "/dance"}:
            headers = None
            ...
            page = {"/operator": "operator.html", "/display": "display.html",
                    "/dance": "dance.html"}.get(path, "operator.html")
