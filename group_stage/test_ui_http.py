...
class UiHttpTest(unittest.TestCase):
    ...
    def test_token_mints_cookie_then_cookie_opens_operator(self):
        status, headers, body = self.request("/operator?token=" + TOKEN)
    ...
    def test_pad_route_removed(self):
        """group_stage 에는 관객(pad) 화면이 없다 — 라우트 자체가 404."""
        _, headers, _ = self.request("/?token=" + TOKEN)
        cookie = headers["Set-Cookie"].split(";", 1)[0]
        self.assertEqual(self.request("/pad", cookie)[0], 404)
